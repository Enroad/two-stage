...
def lowlight(image_path,imageVAN_path, model_path, resultimage_path):
    ...
    light_net=towstagemodel.light_net()
    ...
    img_highlight_lab[:, 0, :,:] = img_highlight_lab[:, 0,:, :] *100*6
    img_highlight_lab[:, 1:,:, :] = img_highlight_lab[:,1:, :, :] * 128.0*4
    ...
    print(end_time)
    image_path = image_path.replace('low', resultimage_path)
...
if __name__ == '__main__':
    # test_images
    with torch.no_grad():
        filePath = 'E:\ye\lol_dataset\our485/low'
        # filePath = 'data/LSRW/'
        model_path = 'E:\ye\codes_SelfDACE\ZeroDE_Lab\EXP\Train-20240718-211620\model_epochs\weights_4.pth'
        resultimage_path = 'ZeroDE_Lab'
        ...
        for image in test_list:
            print(image, image.replace("low", "low_van"))
            lowlight(image, image.replace("low", "low_van"), model_path, resultimage_path)
